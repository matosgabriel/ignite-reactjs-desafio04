...
function Dashboard() {
  ...
  const handleAddFood = async (food: FoodProps) => {
    try {
      const { data: newFoods } = await api.post('/foods', {
        ...food,
        available: true,
      });

      setFoods(defaultValue => [...defaultValue, newFoods]);
    } catch (err) {
      console.log(err);
    }
  }
  ...
  const toggleModal = () => {
    setModalOpen(defaultValue => !defaultValue);
  }

  const toggleEditModal = () => {
    setEditModalOpen(defaultValue => !defaultValue);
  }
